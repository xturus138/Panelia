...
export default function BrowseError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => { console.error('Browse error:', error); }, [error]);

  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center px-4 text-center">
      <AlertCircle className="mb-4 h-10 w-10 text-destructive" />
      <h2 className="mb-1 text-lg font-bold text-foreground">Browse unavailable</h2>
      <p className="mb-5 max-w-sm text-sm text-muted-foreground">{error.message || 'Could not load sources.'}</p>
